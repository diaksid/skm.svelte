<script lang="ts">
  ...
  export let infographic: any[]; // : Infographic[]
  ...
  onMount(() => {
    let delay = 0;
    infographic.forEach(
      (info) => info.max && (delay = info.max > delay ? info.max : delay)
    );
    delay = delay * 10;
...
<div
  ...
  {#each infographic as info}
    <figure class="flex flex-col items-center opacity-30" bind:this={info.node}>
      <img
        class="mb-2"
        src={info.icon.src}
